...
class google_person(IMDB):
	...
	def __init__(self, image_set, root_path, devkit_path):
		...
		"""
		super(google_person, self).__init__('google_' + image_set, root_path)

		self._config = {'comp_id': 'comp4',
						'use_diff': False,
						'min_size': 2}
		self._class_to_ind = dict(zip(self.classes, range(self.num_classes)))
		self._image_index_file = os.path.join(devkit_path, 'indexs', image_set + '.txt')
		self._image_file_tmpl = os.path.join(devkit_path, 'images', '{}.jpg')
		self._image_anno_tmpl = os.path.join(devkit_path, 'labels', '{}.xml')

		# results
		result_folder = os.path.join(devkit_path, 'results', 'person')
	...
	def _parse_voc_anno(filename):
		...
		for obj in tree.findall('object'):
			...
			if int(float(bbox.find('xmin').text))<=0:
				obj_dict['bbox'][0]+=1
			if int(float(bbox.find('ymin').text))<=0:
				obj_dict['bbox'][1]+=1
			if int(float(bbox.find('xmax').text))<=0:
				obj_dict['bbox'][2]+=1
			if int(float(bbox.find('ymax').text))<=0:
				obj_dict['bbox'][3]+=1
